...
public class Ex01 {
	...
	public static void main(String[] args) {
		...
		// 자바에서는 primitive를 제외한 모든 유형의 데이터는 참조변수를 활용한다
		// 참조변수는 그 자체가 본체는 아니고, 참조하는 대상을 바꿀 수 있다
		
		Scanner sc = new Scanner(System.in);
		int length = 3;
		int[] arr = new int[length];
		int[] tmp;	// 새로운 배열을 위한 참조 변수 (초기값은 없음)		// 마지막 if문에서 추가함
		...
		while(true) {									// 이 코드중 while문이 제일 중요함
			...
			for(int i = 0; i < arr.length; i++) {
				if(arr[i] == 0) {		// 빈칸을 찾아서
					arr[i] = input;		// 입력값을 저장하고		// arr[i]는 값이 아니라 칸을 말한다
					break;				// for 탈출
				}
			}
