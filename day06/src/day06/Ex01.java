...
public class Ex01 {
	...
	public static void main(String[] args) {
		// 배열(array) : 같은 자료형의 여러 데이터를 묶어서 관리하는 자료구조		// 배열의 핵심 : 연속된 공간에 배치한다
		// 배열은 여러값을 구별하기 위해 index라고 하는 정수 값을 이용한다
		// 배열의 각 멤버 요소는 단일 변수와 동일하다
		
		int[] arr = { 2, 7, 8, 4, 6 };			// 배열은 같은 자료형만 묶을 수 있다			// 배열의 길이는 절대 바뀌지 않는다
		
		// arr배열의 0번째 요소(변수)의 값은 2이다			// arr(200) + (int) * index		// 배열의 index는 항상 0부터 시작한다(시작지점)
