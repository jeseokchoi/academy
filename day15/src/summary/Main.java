...
public class Main {
	...
	// 	  Animal 의 서브클래스들은 타입에 맞는 먹이 종류를 지정하여 반환하는 함수 getFeed()를 가져야 합니다
	...
	public static void main(String[] args) {
		...
		// Cat과 Dog는 슈퍼클래스 Animal을 상속 받도록 처리하세요
		Cat c1 = new Cat("구름이");
		Dog d1 = new Dog("감자");
		Friend f1 = new Friend("하정우");
		
		h1.give(c1);	// 황정민이 구름이에게 츄르를 준다	> (5번)황정민이 구름이에게 츄르를 던져 준다
		h1.give(d1);	// 황정민이 감자에게 개껌은 준다	> (5번)황정민이 감자에게 개껌은 던져 준다
		h1.give(f1);	// 황정민이 하정우에게 주먹밥을 준다 > (5번)황정민이 하정우에게 주먹밥을 건네 준다
	}
}
