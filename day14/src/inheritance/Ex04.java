...
class Student extends Human {
	...
	@Override
	void show() {
		System.out.printf("이름 : %s, 나이 : %d, 점수 : %s\n", name, age, score);
	}
...
public class Ex04 {
	public static void main(String[] args) {
		...
		Student st2 = new Student("최제석", 41, 22);
		...
		Human h1 = st1;	// 학생은 사람이다	// Human 타입의 메서드를 참조한다
		Human h2 = st2;
...
//		System.out.println(h1.score);	// score cannot be resolved or is not a field (참조되지 않는다)
		System.out.println(((Student)h2).score);	// 강제 형변환
//		System.out.println(h3.major);
		System.out.println(((Doctor)h4).major);
		
		// 다형성에 의해 서로 다른 타입으로 참조할 수 있지만, 타입에 따라 참조가능한 필드 범위가 달라진다(근데 메서드는 다르다)
		
		h1.show();	// show() 메서드는 Human타입에 정의되어 있어서 호출이 가능하다
		h2.show();	// 각 객체의 메서드는 서브클래스에 의해 오버라이딩(덮어씌우기)되어있다
		h3.show();	// 호출이 가능하지만, 호출한 후 실행되는 내용은 덮어씌운 내용으로 실행된다
		h4.show();	// 강제 형변환이 없어도 서브 클래스의 내용대로 실행된다
		
		// 오버라이딩이 아닌 일반 메서드는  강제 형변환을 해줘야한다
		((Doctor)h4).heal(h2);	// 다운캐스팅
		
		// 같은 자료형의 여러데이터를 묶어서 저장하는 구조 : 배열
		Human[] arr = { h1, h2, h3, h4};	// 업캐스팅 
